...
# Bloco de código para gerar o Gráfico 2 que é a Trajetórias da bola e do robô em um plano 𝑥𝑦, até o ponto de interceptação
fig, ax = plt.subplots(figsize=(10, 6))  #essa linha cria uma figura em um conjunto de eixos e especifica o seu tamanho
...
ax.plot(tempo,
        trajetoria_bola[:len(trajetoria_robo), 0], #Plota a coordenada x da bola em função do tempo, são selecionadas as posições X da bola até o ponto onde o robô intercepta.
        #adiciona um rótulo para a legenda e suas configurações.
        label="Bola (Posição X)",
        color="blue",
        linewidth=2)
ax.plot(tempo,
        trajetoria_bola[:len(trajetoria_robo), 1], #Plota a coordenada y da bola em função do tempo, são selecionadas as posições y da bola até o ponto onde o robô intercepta.
        #adiciona um rótulo para a legenda e suas configurações.
...
plt.plot(tempo[:indice_interceptacao], #representa os valores de tempo desde o início até o instante de interceptação.
         ...
         linewidth=2)
plt.plot(tempo[:indice_interceptacao], #representa os valores de tempo desde o início até o instante de interceptação.
         trajetoria_bola[:indice_interceptacao, 1] - 
         trajetoria_bola[indice_interceptacao - 1, 1], #calcula a componente vx da velocidade da bola.
         #rótulo e suas configurações
         label="Bola (vx)",
         color="blue",
         linewidth=2)
plt.plot(tempo[:indice_interceptacao], #representa os valores de tempo desde o início até o instante de interceptação.
         trajetoria_bola[:indice_interceptacao, 2] -
         trajetoria_bola[indice_interceptacao - 1, 2], #calcula componente vy da velocidade da bola.
         #rótulo e suas configurações
         label="Bola (vy)",
         color="green",
         linewidth=2)
